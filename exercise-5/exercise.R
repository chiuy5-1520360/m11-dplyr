...
dep.delay.by.month <- flights %>% group_by(month) %>% summarise(delay = mean(dep_delay, na.rm = TRUE)) %>% filter(delay == max(delay))
...
# Hint: you'll have to perform a grouping operation before summarizing your data
arr.delay.by.airport <- flight %>% group_by(airport) %>% summarise(delay = mean(arr_delay, na.rm = TRUE))
